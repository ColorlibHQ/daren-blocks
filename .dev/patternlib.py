#!/usr/bin/env python3
"""Block markup helpers for Daren's pattern generator.
...
- Font sizes are preset slugs for the same reason, and fs() refuses one that
  is not registered.
...
# The font-size slugs build_theme.py registers. A slug that is not registered
# produces a class that styles nothing, and the text silently keeps its
# inherited size.
SIZES = {"x-small", "small", "medium", "large", "x-large", "xx-large", "title", "heading", "display"}


def size_slug(slug):
    if slug not in SIZES:
        raise ValueError("font size %r is not registered %s" % (slug, sorted(SIZES)))
    return slug
...
def separator(style="daren-band", align=None):
    data = {"className": "is-style-" + style}
...
def cover(inner, slug, overlay="dark", dim=60, min_height=None, align="full",
    ...
    url = "<?php echo esc_url( get_theme_file_uri( 'assets/images/%s.webp' ) ); ?>" % slug
    attr_json = attrs(data).replace('"DAREN_IMAGE"', '"%s"' % url)
