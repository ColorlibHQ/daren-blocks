 ...
 * That shipped in dark mode in an earlier theme: scheme.css lifted `primary`
 * with a colour-mix on a slug the palette did not have, the variable went
 * invalid, and all 25 buttons rendered as bare text.
 *
 ...
 *
 *   WP_URL=http://127.0.0.1:9491 node .dev/button-boundary.mjs
 *   DAREN_DARK=1 WP_URL=… node .dev/button-boundary.mjs
 *   DAREN_PATHS=/,/about/ WP_URL=… node .dev/button-boundary.mjs
 *
...
const dark = process.env.DAREN_DARK === '1';
const paths = ( process.env.DAREN_PATHS || '/,/blog/,/about/,/contact/,/category/street-art/,/tag/colour/,/neon-nights-shooting-under-red-light/,/?s=colour,/no-such-page/' ).split( ',' );
...
for ( const path of paths ) {
	...
	const buttons = await page.evaluate( () => {
		return [ ...document.querySelectorAll( '.wp-block-button__link, .wp-element-button' ) ]
			// The header's scheme switch and search button are icons among
			// icons, by design; their boundary is their hit area, not a fill.
			// Their glyphs are `contrast` on the page, measured as text is.
			.filter( ( el ) => el.offsetWidth > 0 && ! el.closest( '.daren-scheme-toggle, .daren-header__search' ) )
			.map( ( el, i ) => {
				el.setAttribute( 'data-drn-bb', String( i ) );
	...
	for ( const b of buttons ) {
		...
		if ( b.onPhoto ) {
			onPhotos++;
			const handle = await page.$( `[data-drn-bb="${ b.i }"]` );
