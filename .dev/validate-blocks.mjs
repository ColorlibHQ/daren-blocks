...
const problems = await page.evaluate( async () => {
	...
	for ( const pattern of patterns ) {
		if ( ! pattern.name || 0 !== pattern.name.indexOf( 'daren/' ) ) {
			continue;
		}
	...
	// importer actually wrote — including the demo's posts.
	for ( const type of [ 'pages', 'navigation', 'posts' ] ) {
...
console.log( 'Every block in every template, part, pattern, page, menu and post is valid.' );
