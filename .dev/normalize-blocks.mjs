...
// DAREN_PATTERNS says where the patterns actually are in that case.
const patterns = process.env.DAREN_PATTERNS
	? resolve( process.env.DAREN_PATTERNS )
...
// `<?php ... ?>` -> `DRNPHP0`, and back again after the round trip.
function stash( content ) {
	...
	const stashed = content.replace( /<\?php[\s\S]*?\?>/g, ( match ) => {
		...
		return 'DRNPHP' + seen.get( match ) + 'X';
	} );
...
function restore( content, snippets ) {
	return content.replace( /DRNPHP(\d+)X/g, ( _, i ) => snippets[ Number( i ) ] );
}
