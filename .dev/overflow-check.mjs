 ...
 * nothing else — and an earlier theme's booking form inputs were 34px wider than their
 ...
 *
 *   WP_URL=https://example.com DAREN_PATHS=/,/contact/ node .dev/overflow-check.mjs
 *
 * @package Daren
 */
...
const site = process.env.WP_URL || 'http://127.0.0.1:9491';
const paths = ( process.env.DAREN_PATHS || '/,/blog/,/about/,/contact/,/category/street-art/,/tag/colour/,/neon-nights-shooting-under-red-light/,/?s=colour,/no-such-page/' ).split( ',' ).filter( Boolean );
const widths = ( process.env.DAREN_WIDTHS || '1400,1024,768,390' ).split( ',' ).map( Number );
