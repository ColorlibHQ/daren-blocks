 ...
 * call our filter.
 */
const DAREN_UPDATE_HOST     = 'updates.colorlib.com';
const DAREN_UPDATE_ENDPOINT = 'https://updates.colorlib.com/theme/daren.json';
const DAREN_UPDATE_CACHE    = 'daren_update_response';
 ...
 * @return bool
 */
function daren_updates_enabled() {
	/**
	 * Filters whether Daren checks for updates.
	 *
	 ...
	 */
	return (bool) apply_filters( 'daren_check_for_updates', true );
}
 ...
 * @return array
 */
function daren_update_payload() {
	$payload = array(
		'theme'      => 'daren',
		'version'    => DAREN_VERSION,
	 ...
	 */
	return apply_filters( 'daren_update_payload', $payload );
}
 ...
 * @return array|null
 */
function daren_fetch_update() {
	if ( ! daren_updates_enabled() ) {
		return null;
	}

	$cached = get_site_transient( DAREN_UPDATE_CACHE );
	...
	$response = wp_remote_get(
		add_query_arg( daren_update_payload(), DAREN_UPDATE_ENDPOINT ),
		array(
			'timeout'    => 8,
			'user-agent' => daren_update_user_agent(),
		)
	);

	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		set_site_transient( DAREN_UPDATE_CACHE, array(), HOUR_IN_SECONDS );
		return null;
	}
	...
	set_site_transient( DAREN_UPDATE_CACHE, $data, 12 * HOUR_IN_SECONDS );
 ...
 * @return array|false
 */
function daren_check_update( $update, $theme_data, $theme_stylesheet ) {
	if ( $update || 'daren' !== $theme_stylesheet ) {
		return $update;
	}

	$release = daren_fetch_update();

	if ( ! $release || version_compare( $release['version'], DAREN_VERSION, '<=' ) ) {
		return $update;
	}

	return array(
		'id'           => DAREN_UPDATE_ENDPOINT,
		'theme'        => $theme_stylesheet,
		'version'      => $release['version'],
		'url'          => isset( $release['url'] ) ? $release['url'] : 'https://colorlib.com/wp/themes/daren/',
...
add_filter( 'update_themes_' . DAREN_UPDATE_HOST, 'daren_check_update', 10, 3 );

/**
 * Forget the cached response when someone asks WordPress to check again.
 */
function daren_flush_update_cache() {
	delete_site_transient( DAREN_UPDATE_CACHE );
}
add_action( 'upgrader_process_complete', 'daren_flush_update_cache' );
add_action( 'after_switch_theme', 'daren_flush_update_cache' );
...
function daren_updates_notice() {
	...
	$payload = daren_update_payload();

	// Daren ships no companion plugin, so this speaks only for the theme. If one
	...
	$note = __( 'Daren checks %1$s for updates twice a day and sends %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch it off.', 'daren' );
	?>
	<p class="description daren-updates-note">
		<?php
		printf(
			esc_html( $note ),
			'<code>' . esc_html( DAREN_UPDATE_HOST ) . '</code>',
			'<code>' . esc_html( implode( ', ', array_keys( $payload ) ) ) . '</code>',
			'<code>daren_check_for_updates</code>'
		);
		?>
	</p>
	<style>.daren-updates-note { max-width: 70ch; margin-top: 18px; }</style>
	<?php
}
add_action( 'admin_footer', 'daren_updates_notice' );
 ...
 * @return string
 */
function daren_update_user_agent() {
	return 'Daren/' . DAREN_VERSION . '; WordPress/' . get_bloginfo( 'version' );
}
 ...
 * @return array
 */
function daren_update_request_args( $args, $url ) {
	if ( 'updates.colorlib.com' === wp_parse_url( $url, PHP_URL_HOST ) ) {
		$args['user-agent'] = daren_update_user_agent();
	}
	return $args;
}
add_filter( 'http_request_args', 'daren_update_request_args', 10, 2 );
